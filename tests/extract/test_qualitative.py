...
from tearsheet.store.models import Document, ExtractedSpan, Citation, Filing
...
class TestSemanticChunking:
    ...
    def test_extract_risk_factors_large_document(self):
        ...
        mock_llm.complete_structured.return_value = RiskList(risks=[
            RiskFactor(exact_quote="large document risks.")
        ])
...
def test_extract_risk_factors():
    # Setup mock document
    filing = Filing(company_id=10)
    doc = Document(id=42, filing=filing, text="Liquidity Risk. We face significant liquidity risks.", section="1A")

    # Setup mock LLM Client
    mock_llm = MagicMock()

    # Return a RiskList with one valid (labelled) and one hallucinated quote
    mock_llm.complete_structured.return_value = RiskList(risks=[
        RiskFactor(exact_quote="liquidity risks.", label_quote="Liquidity Risk."),
        RiskFactor(exact_quote="market volatility risks.") # Hallucination
    ])
    ...
    # Label is a verified source slice with its own offsets
    assert span.label == "Liquidity Risk."
    assert doc.text[span.label_start_offset:span.label_end_offset] == "Liquidity Risk."

    assert len(span.citations) == 1
    citation = span.citations[0]
    assert citation.quote == "liquidity risks."
    assert citation.document_id == 42
    assert doc.text[citation.start_offset:citation.end_offset] == "liquidity risks."
...
def test_extract_business():
    ...
    mock_llm.complete_structured.return_value = BusinessProfile(
        revenue_streams=[GroundedItem(exact_quote="sell cloud services.")],
        competitors=[GroundedItem(exact_quote="compete with AWS.")],
        moats=[
            GroundedItem(exact_quote="scale is a moat."),
            GroundedItem(exact_quote="sell cloud services.") # Duplicated quote!
        ]
    )
...
def test_extract_mda():
    ...
    mock_llm.complete_structured.return_value = MDAnalysis(
        liquidity=[GroundedItem(exact_quote="Cash is strong.")],
        kpis=[GroundedItem(exact_quote="EBITDA grew.")],
        forward_sentiment=[GroundedItem(exact_quote="expect good things.")]
    )
